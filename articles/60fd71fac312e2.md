...
しかしその補完を確定するためにEnterを押すべきかどうかはわかりません。
...
ちなみに、AutoHotkeyのバージョン2に合わせて先程の関数群を更新している方がおり、IME_GET関数はそこから使わせていただきました。私もバージョン2のAutoHotkeyで書くことにしたので、助かりました。
...
```
という部分を他のファイル名に変えれば、DiscordだけでなくGPTやClaudeなどの生成AIを使う際にも同じキーバインドを適用することができると思います。
