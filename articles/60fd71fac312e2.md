...
この`.ahk`ファイルをレジストリエディタのRunに登録します。

スタートアップとして登録しても良いのですが、ログオン後数分しないと起動しないので、レジストリエディタに登録してしてすぐ起動させた方が使い勝手が良いです。

そうすればPCの電源を入れログオンした直後に自動起動するようになります。

## 設定方法

1. AutoHotkeyのv2.0をインストール

https://www.autohotkey.com/

2. `"C:\Users\ユーザ名\AppData\Local\AHK\my_shortcut.ahk"`などのパスに`.ahk`ファイルを移動しておく

3. Winキーを押して「レジストリ エディター」と入力し、クリック

4. 以下パスに移動

`コンピューター\HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`

5. 4のパスで右クリック->新規->文字列値(S)で値を作成
    - 「名前」は好きな名前に(AHK_my_shortcutなど)指定
    - 「種類」はデフォルトでREG_SZになっている
    - 「値」を先ほど`.ahk`ファイルを移動したパスに指定
    - `"C:\Users\ユーザ名\AppData\Local\AHK\my_shortcut.ahk"`

6. 以下パスに移動

`コンピューター\HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\Serialize`

7. 6のパス内に`DWORD (32ビット) 値 (D)`を2つ作る
...
Run内に入れるだけだと起動が遅かったので、WaitForIdleStateも入れてみたところ起動がかなり早くなりました。一応StartupDelayInMSecも入れておくと間違いないと思います。

参考

https://learn.microsoft.com/ja-jp/answers/questions/5540790/windows11
