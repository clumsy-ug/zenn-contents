...
# 使い方
PCの電源を入れるたびに毎回`.ahk`ファイルをダブルクリックして実行するという方法でも使えますが、それは面倒ですよね。

そこで、この`.ahk`ファイルをスタートアップとして登録することで、PC起動時に自動的に起動するようにします。
...
今回の機能を自分自身で普段使いしていくつもりなので、使っていく中で変な挙動を確認したらその都度修正していこうと思います。

# 追記
### 11/17
- Ctrl + V(コピペ)->Enter  で送信されてしまう挙動を修正しました。
- クリック(補完確定)->Enter  で送信されてしまう挙動を修正しました。
- コピペ->全角スペース->Enter  で送信されてしまう挙動を修正しました。

### 11/18
- Ctrl + Enter(補完確定)->Enter  で送信されてしまう挙動を修正しました。
- 全角の`￥`、`：`、`＾`、`～`、`｜`、`‘`、`＊`、`＿`がEnterで補完確定できない挙動を修正しました。
- 英語で入力->CapsLockでひらがなモードに変更->Enter  で送信さえてしまう挙動を修正しました。
