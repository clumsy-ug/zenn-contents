...
# 追記
### 11/17
- Ctrl + V(コピペ)->Enterで送信されてしまう挙動を修正しました。
- クリック(補完確定)->Enterで送信されてしまう挙動を修正しました。
- コピペ->全角スペース->Enterで送信されてしまう挙動を修正しました。

### 11/18
- Ctrl + Enter(補完確定)->Enterで送信されてしまう挙動を修正しました。
- 全角の`￥`、`：`、`＾`、`～`、`｜`、`‘`、`＊`、`＿`がEnterで補完確定できない問題を修正しました。
