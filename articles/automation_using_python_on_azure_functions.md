...
また、1回でもdropが走るとdfの長さ(`len`)が変わり最初の`if df.at[n, name_column] == 'NULL':`の部分のn行目がdf内に存在しなくなってしまうことで最後の方にKeyErrorが出るかもしれないと思いましたが、その心配はなさそうです。

まず`drop`は指定された行(n)を削除しますが、残りの行のインデックスを詰める（振り直す）ことはしません。
例えば、インデックスが 0, 1, 2 と並んでいる状態で 0 を削除しても、残りの行は 1, 2 のままです（0, 1 に振り直されるようなことはありません）。

そのためそのようなKeyErrorは起こらないと言えます。

とはいえ、もっと効率化したコードにできると思います。今回はあえて素直なアルゴリズムを自力で実装してみたかったのでこのように書きました。
...
これまでと同様openpyxlで実現したかったのですがなぜかうまくいかなかったので、`.xlsx`の中身である`.xml`を直接操作することにしました。多分ダーティハックです。
...
また、グラフのタイトルがExcelファイルのUI上は見えているのに、対応する中身である`.xml`ファイルを見てみるとタイトルが存在しないことになっているという事象に遭遇しました。
ここから考えられることは、ExcelファイルにおいてxmlとUIは完全に一致している訳ではなく、Excelは内部で一時キャッシュのようなものを恐らく持っており、それが見た目に寄与することがあるということです。
